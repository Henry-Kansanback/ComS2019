...
static void io_redisplay_non_terrain_aiming(dungeon *d, pair_t cursor)
{
...
uint32_t io_aimranged(dungeon *d, int inkey)
{
  pair_t reader;
  int c;
  fd_set readfs;
  ...
  //io_display(d);
  mvprintw(0, 0,
           "Hover over a monster.  't' to fire; 'Q' for escape.");

  reader[dim_y] = d->PC->position[dim_y];
  reader[dim_x] = d->PC->position[dim_x];

  mvaddch(reader[dim_y] + 1, reader[dim_x], '*');
  refresh();
  ...
  do {
    do{
      ...
      io_redisplay_non_terrain_aiming(d, reader);
      //io_redisplay_non_terrain(d, reader);
    } while (!select(STDIN_FILENO + 1, &readfs, NULL, NULL, &tv));
    /* Can simply draw the terrain when we move the cursor away, *
     * because if it is a character or object, the refresh       *
     * function will fix it for us.                              */
    switch (mappair(reader)) {
    case ter_wall:
    case ter_wall_immutable:
    case ter_unknown:
      mvaddch(reader[dim_y] + 1, reader[dim_x], ' ');
 ...
 /* Use zero as an error symbol, since it stands out somewhat, and it's *
  * not otherwise used.                                                 */
      mvaddch(reader[dim_y] + 1, reader[dim_x], '0');
    }
    switch ((c = getch())) {
    case '7':
    case 'y':
    ...
    if(c == 't' && inkey == 'r')
      {
	if (charpair(reader) && charpair(reader) != d->PC && can_see(d,
								     character_get_pos(d->PC),
	  							     character_get_pos(d->character_map[reader[dim_y]][reader[dim_x]]), 1, 0)) 
	  {
	    rangedAttack(d, d->PC, d->character_map[reader[dim_y]][reader[dim_x]]);
	    if(d->PC->equipped[2])
	      {
		//mvprintw(4, 9, "You hit %s!", d->character_map[reader[dim_y]][reader[dim_x]]->name);
		break;
	      }
	  }
	else
	  {
	    mvprintw(4, 9, "Out of Range");
	...
	int spelltype = getch();
	/*
	  mvprintw(10, 9, "Select a, b or c for spell type");
	  mvprintw(11, 9, "a for Wand, b for Scroll, and c for Book");
	  mvprintw(12, 9, "Wand for sngl target, Scroll for higher dmg, Book for 3x3 block dmg");
	*/
	mvprintw(17, 9, "Press i for instructions, m for spell costs, and g to clear messages");
	mvprintw(18, 9, "Mana available: %d", d->PC->mana);
	// 'a', 'b', 'c' for wand, scroll and book, respectively. 
	while(spelltype != 'a' && spelltype != 'b' && spelltype != 'c' && spelltype != 27 && spelltype != 'Q')
	  {
	    spelltype = getch();
	    if(spelltype == 'i')
	    {
	    ...
	    if(spelltype == 'm')
	      {
		//mvprintw(2, 9, "Current Mana:")
		mvprintw(3, 9, "Wand: %d, Scroll: %d, Book: %d", costTest(d, 'a'), costTest(d, 'b'), costTest(d, 'c'));
	      }
	    if(spelltype == 'g')
	      {
		io_display(d);
		...
		mvprintw(17, 9, "Press i for instructions, d for spell costs, and g to clear messages");
		mvprintw(18, 9, "Mana available: %d", d->PC->mana);
	      }
	  }
	if(spelltype != 27 && spelltype != 'Q')
	  {
	    sCost = costTest(d, spelltype);
	    /*
	    if(d->PC->equipped[12])
	      {
...
*/
	    ...
	    if (d->PC->mana > sCost) 
	      {
		if((spelltype == 'a' || spelltype == 'b') && d->character_map[reader[dim_y]][reader[dim_x]] 
		   && can_see(d, character_get_pos(d->PC), 
			      character_get_pos(d->character_map[reader[dim_y]][reader[dim_x]]), 1, 0))
		  {
		    spellAttack(d, d->PC, d->character_map[reader[dim_y]][reader[dim_x]], spelltype, sCost);
		    break;
		  }
		else if(spelltype == 'c' && is_illuminated(d->PC, reader[dim_y], reader[dim_x]))
		  {
		    blastArea(d, reader, sCost);
		    break;
		  }
		else if(!is_illuminated(d->PC, reader[dim_y], reader[dim_x]) || !can_see(d, character_get_pos(d->PC), character_get_pos(d->character_map[reader[dim_y]][reader[dim_x]]), 1, 0))
		  {
		    mvprintw(4, 9, "Location invalid");
		  }
		  //break;
		//mvprintw(4, 9, " %-60s ", charpair(reader)->description);
	      }
	    else
	      {
		//mvprintw(4, 9, "Out of Range");
		//mvprintw(5, 9, "or Not enough Mana");
		//io_queue_message("No monster in this position.");
	      }
	  }
      }
  } while (c != 'Q' && c != 27);
...
uint32_t io_lookat(dungeon *d)
{
  ...
  mvprintw(0, 0,
           "Hover over a monster.  't' to lookat; 'Q' for escape.");
  ...
  * not otherwise used.                                                 */
    ...
    if(c == 't')
      {
	if (charpair(reader) && charpair(reader) != d->PC) 
	  {
	    mvprintw(4, 9, " %-60s ", charpair(reader)->description);
	  }
	else
...
//while (c != 't' && c != 'Q');
  ...
  /*if (c == 'r') {
    do {
      reader[dim_x] = rand_range(1, DUNGEON_X - 2);
      reader[dim_y] = rand_range(1, DUNGEON_Y - 2);
    } while (charpair(reader) || mappair(reader) < ter_floor);
  }*/
  /*
  if (charpair(reader) && charpair(reader) != d->PC) {
    //Display description
    ...
    //mvprintw();
    mvprintw(4, 9, " %-60s ", charpair(reader)->description);
    //io_queue_message("Teleport failed.  Readerination occupied.");
    //d->character_map[d->PC->position[dim_y]][d->PC->position[dim_x]] = NULL;
    //d->character_map[reader[dim_y]][reader[dim_x]] = d->PC;

    //d->PC->position[dim_y] = reader[dim_y];
    //d->PC->position[dim_x] = reader[dim_x];
  } else { 
    io_queue_message("No monster in this position.");
    }*/
...
static const char itemnum[] = {
  'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm'
};



static void io_list_equip_display(dungeon *d,
                                     object **c,
				  uint32_t count, int key)
{
  uint32_t i;
  char (*s)[60]; /* pointer to array of 60 char */
  char tmp[41];  /* 19 bytes for relative direction leaves 40 bytes *
  ...
  for (i = 0; i < 13; i++) {
    if(d->PC->equipped[i])
      {// added space after second :
    snprintf(tmp, 41, "%3c: %s (%c):  ", itemnum[i],
             c[i]->get_name(),
             c[i]->get_symbol());
      }
    else
      {
	snprintf(tmp, 41, "%3c: %s", itemnum[i], "Empty");
      }

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat"

    snprintf(s[i], 60, "%40s", tmp);
#pragma GCC diagnostic pop
    if (count <= 13) {
	...
	while(!((key) <= 'm') || key != 27)
	  {
		...
		break;
	      ...
	      {
		mvprintw(count + 7, 9, " %-60s ", "Press a key denoting an equipt slot.");
	      }
	    key = getch();
	  }
	break;
...
static void io_list_equip(dungeon *d , int key)
{
  object **c;
  //uint32_t x, y, count;
  uint32_t count;
  c = (object **) malloc(13 * sizeof (*c));

  //count = 0;
  for(count = 0; count < 13; count++)
    {
      if(d->PC->equipped[count])
	{
	  c[count] = d->PC->equipped[count];
	}
    }
  /* Sort it by distance from PC */
  thedungeon = d;
  ...
  count = 13;
  /* Display it */
  io_list_equip_display(d, c, count, key);
  free(c);
...
static void io_list_held_display(dungeon *d,
                                     object **c,
				  uint32_t count, int key)
{
  uint32_t i;
  ...
  for (i = 0; i < 10; i++) {
    if(d->PC->held[i] != NULL){
      snprintf(tmp, 41, "%3d: %3s (%c): ", i, c[i]->get_name(), c[i]->get_symbol());
      //snprintf(tmp, 41, "%3d%s (%c): ", i, d->PC->held[i]->get_name(), d->PC->held[i]->get_symbol());
    }
    else{
      snprintf(tmp, 41, "%3d: %3s:", i, "Empty");
    }
...
#pragma GCC diagnostic pop
  ...
  if (count <= 13) {
	...
	while(((key) <=  57) || key != 27)
	  {
	    if((key > 47) && (key <= 57) && !(d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]]) && key != 27)
	      {
		//unequip(d, key);
		//io_queue_message()
		drop(d, key - 48);
		break;
	      ...
	      {
		mvprintw(count + 7, 9, " %-60s ", "Press a key denoting an carry slot.");
	      }
	    if(key == 27)
	      {
		break;
	...
	while((key != 27))
	  {
		...
		if(d->PC->held[key - 48]) 
		  {
		    mvprintw(4, 9, " %-10s ", d->PC->held[key - 48]->get_descr());
		    break;
		  }
		else
		  {
		    
		    //io_queue_message("No object in this slot.");
		  }
	      }
	    key = getch();
	  }
	...
	while((key != 27) || (key <= 57))
	  {
	    key = getch();
	    if((key <= 57) && (key > 47) && d->PC->held[key - 48]){
	      mvprintw(count + 7, 9, "You've destroyed %s ", d->PC->held[key - 48]->get_name());
	      d->PC->held[key - 48] = NULL;
	      d->PC->numheld--;
	      //key = 27;
	      break;
	    }
	...
	while((key <= 57) || (key != 27))
	  {
		...
		if(d->PC->held[key - 48])
		  ...
		  {
		    mvprintw(count + 7, 9, "No item in this slot");
		  }
	      }
	    if(key == 27)
	      {
...
static void io_list_held(dungeon *d , int key)
{
  object **c;
  //uint32_t x, y, count;
  uint32_t count;
  c = (object **) malloc(10 * sizeof (*c));
  ...
  count = 10;
  /* Display it */
  io_list_held_display(d, c, count, key);
  free(c);
...
void io_handle_input(dungeon *d)
{
  ...
  if(d->PC->mana < d->PC->manacap)
    {
      uint8_t tempmana;
      if((tempmana = d->PC->mana + d->PC->manaregen) > d->PC->manacap)
	{
	  d->PC->mana = d->PC->manacap;
	}
      else
	{
	  d->PC->mana = tempmana;
